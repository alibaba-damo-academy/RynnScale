"""AgileX Cobot Magic 2 renderer (two Piper 6-DOF arms).

Uses the same ``piper_description`` MJCF as
:class:`~.agilex_split_aloha.AgilexSplitAlohaRenderer` (from the
``split_aloha`` branch of ``agilexrobotics/mobile_aloha_sim``).

Joint3 sign is inconsistent across the data feeding this renderer: some cells
store it in the Piper SDK convention (j3 > 0), others already in the model's
URDF convention (j3 ∈ [-2.967, 0]). RoboCOIN's ``Agilex_Cobot_Magic_*`` and
~70%% of RoboMIND's agilex tasks are already in URDF convention; the rest are
SDK convention. Since the elbow is physically one-sided, the sign alone
identifies the convention: any positive j3 is SDK-convention and is flipped to
the model's, while non-positive j3 is applied as-is. This normalizes both
without a per-source flag.
"""
...
# Elbow joint index within the 6-DOF arm; sign identifies SDK vs URDF convention.
_J3_INDEX = 2


@RENDERER_REGISTRY.register(name=RobotType.AGILEX_COBOT_MAGIC_2.value)
class AgilexCobotMagic2Renderer(BaseRenderer):
    """Dual Piper renderer for AgileX Cobot Magic 2 data."""
    ...
    def _apply_arm(
        ...
        if self._use_joint(arm):
            ...
            # Normalize joint3 to the model's URDF convention. A positive value
            # is Piper SDK convention (mirror of the model's [-2.967, 0] elbow
            # range) and must be flipped; a non-positive value is already in
            # the model convention and is left untouched. This applies only to
            # recorded joints — IK already solves in the model convention.
            if qpos[_J3_INDEX] > 0.0:
                qpos[_J3_INDEX] = -qpos[_J3_INDEX]

            self.data.qpos[qadr] = np.clip(qpos, jrange[:, 0], jrange[:, 1])
        else:
            assert arm.eef_position is not None and arm.eef_rotation is not None, (
                "AgilexCobotMagic2Renderer requires joint_position or full EEF pose"
            )
