...
@RENDERER_REGISTRY.register(name=RobotType.TIANYI.value)
class TianyiRenderer(BaseRenderer):
    """Tianyi 2.0 humanoid dual-arm renderer (joint playback).

    7-DOF arms, no dexterous hands. Wheeled base with waist, head, dual arms.
    """
    ...
    def _build_model(self):
        ...
        urdf_path = os.path.join(project_root, "assets", "tianyi", "tianyi2.urdf")
        assert os.path.exists(urdf_path), f"Tianyi 2.0 model not found: {urdf_path}"
    ...
    def _apply_arm(self, arm: Arm, qadr, dofadr, jrange, body_id, prev_qpos):
        if arm.joint_position is not None:
            ...
            assert qpos.shape == (self.N_ARM,), f"TianyiRenderer expects 7-DOF per arm, got shape {qpos.shape}"
        ...
        assert arm.eef_position is not None and arm.eef_rotation is not None, (
            "TianyiRenderer requires joint_position or full EEF pose"
        )
    ...
    def render(self, action: RobotAction) -> np.ndarray:
        ...
        for arm in (action.left_arm, action.right_arm):
            ...
            assert ref is not None, "TianyiRenderer requires either joint_position or eef pose"
