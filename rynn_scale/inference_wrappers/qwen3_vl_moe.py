...
class Qwen3VLMoeInferenceWrapper(Qwen3VLInferenceWrapper):
    def load_model(self):
        from ..models.qwen3_vl_moe.modeling_qwen3_vl_moe import _Qwen3VLMoeTextExperts
        ...
        with torch.no_grad():
            for layer in model.model.language_model.layers:
                if hasattr(layer.mlp, "experts"):
                    gate_up_proj = layer.mlp.experts.gate_up_proj.transpose(1, 2).flatten(start_dim=0, end_dim=1)
                    down_proj = layer.mlp.experts.down_proj.transpose(1, 2).flatten(start_dim=0, end_dim=1)

                    with torch.device("meta"):
                        new_module = _Qwen3VLMoeTextExperts(config=model.model.language_model.config)
